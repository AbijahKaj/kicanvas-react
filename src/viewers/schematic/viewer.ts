...
> {
  ...
  override async load(src: KicadSch | ProjectPage) {
    ...
    // Safely call update_hierarchical_data if it exists
    // Skip warning as this is normal for some document types
    if (doc && typeof doc.update_hierarchical_data === "function") {
      try {
        doc.update_hierarchical_data(src.sheet_path);
      } catch (error) {
        // Silently handle the error - this is expected in some cases
        console.warn("Error updating hierarchical data:", error);
      }
    }
  ...
  public override select(
    ...
    if (is_string(item)) {
      item =
        this.schematic.find_symbol(item) ?? this.schematic.find_sheet(item);
    }
